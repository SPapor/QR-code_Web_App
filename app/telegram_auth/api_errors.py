...
class ApiErrors:
    INVALID_BOT_SECRET = ApiError(
        status.HTTP_401_UNAUTHORIZED, "Invalid bot secret", "telegram_auth.0001"
    )
    BOT_INTEGRATION_DISABLED = ApiError(
        status.HTTP_404_NOT_FOUND, "Bot integration disabled", "telegram_auth.0002"
    )
...
def register_exception_handlers(app: FastAPI):
    ...
    static_exception_handler(
        app, TargetAccountAlreadyLinkedError, ApiErrors.TARGET_ACCOUNT_ALREADY_LINKED
    )
